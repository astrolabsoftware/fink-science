...
def extract_features_ad_rubin_raw(
    ...
    df = df.sort_values('time').drop_duplicates(subset='time') # Just in case, we delete duplicates, in case there is the same problem as with ZTF.
    ...
    for filter_name in unique_bands:
        ...
        try:
            ...
            logger.exception(f"Unknown exception for {objectId} in band {filter_name}: {e}")
            continue
...
if __name__ == "__main__":
    ...
    assert len(features[1]) == len(FEATURES_COLS), "Incorrect number of features extracted"
