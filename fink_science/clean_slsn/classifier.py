...
def slsn_classifier(data, metadata):
    ...
    formated = base.format_data(data, 'ELASTICC')
    ...
    all_features = fe_slsn.parametrise(all_transformed, metadata)

    features = fe_slsn.merge_features(all_features, metadata)

    clf = load_classifier(metadata)

    proba = base.get_probabilities(clf, features, valid)
